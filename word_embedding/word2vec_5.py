...
if __name__ == '__main__':
    ...
    embed_size = 256
    print('find_data...')
    creative_list = merge_df['text'].parallel_apply(creative_t).to_list()
    print(len(creative_list))
    # print('开始序列化')
    # # 适当修整长度
    # x1, index_1 = set_tokenizer(category_list, split_char=' ', max_len=max_len)
    # print('序列化完成')
    trian_save_word2vec(creative_list, window=int(max_len / 2), save_name=config.model_path + 'w2v_model/{}_256.txt'.format(flag),
                        split_char=' ')
