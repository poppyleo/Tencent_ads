...
if __name__ == '__main__':
    ...
    error_df = pd.read_csv(config.new_data_process_quarter_final + 'error_entities.csv', encoding="utf-8-sig")  # 训练集的预测结果与真实结果的并集
    error_df = error_df.fillna('')
    error_list = list(set(error_df['errorEntities'].tolist()))
    print(error_list)
